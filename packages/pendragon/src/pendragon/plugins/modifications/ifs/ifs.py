...
class IFSTransform(BaseModel):
    ...
    variation: Literal["linear", "sinusoidal", "spherical", "swirl"] = Field(
        default="linear",
        description=
        "The non-linear math function applied after the affine transform.")
...
class IFSMod(PendragonOperation):
    ...
    def apply_variation(self, x: float, y: float,
                        var_type: str) -> Tuple[float, float]:
        """
    ...
    def process(self,
        ...
        logger.info(
            f"Applying Advanced IFS ({len(cfg.transforms)} transforms, {iterations} iterations)..."
        )
        ...
        for i in range(iterations):
            ...
            logger.debug(
                f"IFS Iteration {i+1} complete: {len(working_lines)} paths.")

        logger.success(
            f"IFS generation complete. Yielded {len(working_lines)} lines.")
    ...
    def build_custom_ui(self, window, op_index):
        ...
        def trigger_recalc():
            if window._pending_op_index is None:
                ...
                window._pending_op_index = min(window._pending_op_index,
                                               op_index)
        ...
        list_group.setStyleSheet(
            "QGroupBox { border: 1px solid #444; border-radius: 4px; padding-top: 15px; }"
        )
        ...
        for item_idx, transform in enumerate(self.config.transforms):
            frame = QFrame()
            frame.setStyleSheet(
                "QFrame { background-color: #252526; border: 1px solid #333; border-radius: 4px; }"
            )
            ...
            btn_remove.setStyleSheet(
                "background-color: #5a1c1c; border: none; padding: 4px; border-radius: 2px;"
            )
            btn_remove.setFixedWidth(60)
            btn_remove.clicked.connect(
                lambda checked, i=item_idx: remove_transform(i))
            ...
            labels = [
                "a (scale X)", "b (shear Y)", "d (shear X)", "e (scale Y)",
                "X Offset", "Y Offset"
            ]
        ...
        btn_add.setStyleSheet(
            "background-color: #2a2d32; color: #4CAF50; font-weight: bold; border: 1px dashed #555;"
        )
