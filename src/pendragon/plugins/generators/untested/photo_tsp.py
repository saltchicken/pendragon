...
class PhotoTSPConfig(BasePluginConfig):
    nodes: int = Field(default=2000, gt=1, description="Number of points to connect.")
...
class PhotoTSPGen(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        if not cfg.image_path: return state

        nodes = int(ctx.variables.get("nodes", cfg.nodes))
        logger.info(f"Generating {nodes}-node TSP path from {cfg.image_path}...")
        ...
        while len(points) < nodes and batches < max_batches:
            batches += 1
            xs, ys = np.random.uniform(minx, maxx, batch_size), np.random.uniform(miny, maxy, batch_size)
            rand_thresholds = np.random.random(batch_size)
            
            for i in range(batch_size):
                if len(points) >= nodes: break
                x, y = xs[i], ys[i]
        ...
        for _ in range(len(pts) - 1):
            ...
            while best_idx == -1:
                ...
                if query_k == 1: idxs = [idxs]
                for idx in idxs:
                    if not visited[idx]:
                        best_idx = idx
                        break
                if best_idx == -1:
                    if query_k == len(pts): break
                    k *= 4
            if best_idx != -1:
        ...
        return PipelineState(boundary=state.boundary, lines=state.lines + [final_path], operation_name="photo_tsp")
