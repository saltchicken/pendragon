...
class HilbertConfig(BasePluginConfig):
    spacing: float = Field(default=2.0, gt=0.0, description="Target distance between segments.")
...
class HilbertGen(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        order = int(math.ceil(math.log2(size / safe_spacing))) if size > safe_spacing else 1
        order = min(order, 8)

        logger.info(f"Generating Hilbert curve (order={order}, spacing={spacing})...")

        def hilbert(x0, y0, xi, xj, yi, yj, n):
            if n == 0: return [(x0 + (xi + yi) / 2.0, y0 + (xj + yj) / 2.0)]
            return (
        ...
        return PipelineState(boundary=state.boundary, lines=state.lines + clipped_lines, operation_name="hilbert")
