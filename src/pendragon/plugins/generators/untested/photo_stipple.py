...
class PhotoStippleConfig(BasePluginConfig):
    dots: int = Field(default=5000, gt=0, description="Target number of stipple dots to generate.")
    image_path: str | None = Field(default=None, description="Source image.")
    dot_size: float = Field(default=0.1, gt=0.0, description="Length of microscopic line.")
...
class PhotoStippleGen(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        if len(new_dots) < dots:
            logger.warning(f"Only generated {len(new_dots)} out of {dots} requested dots.")

        logger.success(f"Generated {len(new_dots)} bounded stipple points.")
        return PipelineState(boundary=state.boundary, lines=state.lines + new_dots, operation_name="photo_stipple")
