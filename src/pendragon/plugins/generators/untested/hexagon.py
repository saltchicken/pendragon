...
class HexagonConfig(BasePluginConfig):
    radius: float = Field(default=3.0, gt=0.0, description="Outer radius of a single hexagon cell.")
...
class HexagonGen(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        def quantize(pt): return (int(round(pt[0] * 1000)), int(round(pt[1] * 1000)))
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                for i in range(6):
                    ...
                    vertices.append((cx + radius * math.cos(angle_rad), cy + radius * math.sin(angle_rad)))
        ...
        while unique_edges:
            ...
            if start_node is None:
                ...
                if candidates: start_node = candidates[0]
        ...
        def extract_valid_lines(geom):
            ...
            if geom.geom_type == 'LineString': extracted.append(geom)
            elif geom.geom_type == 'MultiLineString': extracted.extend(list(geom.geoms))
            elif geom.geom_type == 'GeometryCollection':
                for g in geom.geoms:
                    if g.geom_type == 'LineString': extracted.append(g)
                    elif g.geom_type == 'MultiLineString': extracted.extend(list(g.geoms))
            return extracted

        for line in raw_lines:
            if line.intersects(effective_boundary):
                clipped_lines.extend(extract_valid_lines(line.intersection(effective_boundary)))

        logger.success(f"Generated Hexagon fill. Retained {len(clipped_lines)} continuous paths.")
        return PipelineState(boundary=state.boundary, lines=state.lines + clipped_lines, operation_name="hexagon")
