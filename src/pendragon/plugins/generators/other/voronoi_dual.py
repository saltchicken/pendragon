...
class VoronoiDualConfig(BasePluginConfig):
    spacing: float = Field(default=2.0, gt=0.0, description="Target spacing between points.")
    num_points: int = Field(default=0, ge=0, description="Number of random seed points.")
    seed: int = Field(default=42, description="Random seed.")
    mode: str = Field(default='dual', description="Mode: 'voronoi', 'delaunay', or 'dual'.")
    ...
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        if mode in ["voronoi", "dual"]:
            ...
            for ridge in vor.ridge_vertices:
                if -1 not in ridge:
                    raw_lines.append(LineString([vor.vertices[ridge[0]], vor.vertices[ridge[1]]]))
        ...
        if mode in ["delaunay", "dual"]:
            ...
            for simplex in tri.simplices:
                for i in range(3):
                    delaunay_edges.add(tuple(sorted([simplex[i], simplex[(i + 1) % 3]])))
        ...
        return PipelineState(boundary=state.boundary, lines=state.lines + clipped_lines, operation_name="voronoi_dual")
