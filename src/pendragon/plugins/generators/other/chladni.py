...
class ChladniConfig(BasePluginConfig):
    ...
    sign: float = Field(default=-1.0, description="Sign of the wave interference.")
    res: float = Field(default=0.5, gt=0.0, description="Grid sampling resolution.")
    simplify: float = Field(default=0.0, ge=0.0, description="Tolerance for path simplification.")
    image_path: str | None = Field(default=None, description="Optional image modulator.")
...
class ChladniGen(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        logger.info(f"Generating Chladni pattern (n={n_mode}, m={m_mode}) at resolution {res}...")
        ...
        def get_val(x: float, y: float) -> float:
            u = (x - minx) / width
            v = (y - miny) / height
            val = (math.cos(n_mode * math.pi * u) * math.cos(m_mode * math.pi * v) +
                   cfg.sign * math.cos(m_mode * math.pi * u) * math.cos(n_mode * math.pi * v))
            if sampler:
        ...
        for i in range(cols - 1):
            for j in range(rows - 1):
                ...
                v00, v10, v11, v01 = vals[i][j], vals[i + 1][j], vals[i + 1][j + 1], vals[i][j + 1]

                pts = []
                if (v00 > 0) != (v10 > 0): pts.append(interp(v00, v10, (x0, y0), (x1, y0)))
                if (v10 > 0) != (v11 > 0): pts.append(interp(v10, v11, (x1, y0), (x1, y1)))
                if (v11 > 0) != (v01 > 0): pts.append(interp(v11, v01, (x1, y1), (x0, y1)))
                if (v01 > 0) != (v00 > 0): pts.append(interp(v01, v00, (x0, y1), (x0, y0)))
                ...
                if len(pts) == 2:
                    ...
                    if (center_val > 0) == (v00 > 0):
                        segments.extend([LineString([pts[0], pts[3]]), LineString([pts[1], pts[2]])])
                    else:
                        segments.extend([LineString([pts[0], pts[1]]), LineString([pts[2], pts[3]])])

        if not segments: return state

        merged = linemerge(segments)
        if cfg.simplify > 0:
            merged = merged.simplify(cfg.simplify, preserve_topology=False)

        raw_lines = [merged] if isinstance(merged, LineString) else list(merged.geoms) if isinstance(merged, MultiLineString) else segments
        ...
        return PipelineState(boundary=state.boundary, lines=state.lines + clipped_lines, operation_name="chladni")
