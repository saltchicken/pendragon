...
class OffsetFillConfig(BasePluginConfig):
    spacing: float = Field(default=2.0, gt=0.0, description="Distance between offset rings.")
    ring_simplify: float = Field(default=0.2, ge=0.0, description="Simplification tolerance.")
...
class OffsetFillGen(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        logger.info(f"Generating concentric offset fill with spacing {spacing}...")

        new_lines: List[LineString] = []
        current_geom = effective_boundary.buffer(-spacing).simplify(cfg.ring_simplify, preserve_topology=False)
        ...
        while not current_geom.is_empty and current_geom.area > 0:
            ...
            current_geom = current_geom.buffer(-spacing).simplify(cfg.ring_simplify, preserve_topology=False)

        logger.success(f"Offset fill complete. Generated {len(new_lines)} contour paths.")
        return PipelineState(boundary=state.boundary, lines=state.lines + new_lines, operation_name="offset_fill")
