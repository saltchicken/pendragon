...
class PhotoWaveConfig(BasePluginConfig):
    lines: int = Field(default=80, gt=0, description="Number of horizontal wave lines.")
...
class PhotoWaveGen(PipelineOperation):
    ...
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        logger.info(f"Generating {lines_count} photo waves from {cfg.image_path}...")
        ...
        return PipelineState(boundary=state.boundary, lines=state.lines + clipped_lines, operation_name="photo_wave")
