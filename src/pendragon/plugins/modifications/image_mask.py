...
class ImageMaskConfig(BaseModel):
    ...
    sample_step: float = Field(
        default=0.5,
        description="Resolution step size for sampling along lines.")
...
class ImageMaskMod(PipelineOperation):
    ...
    def process(self, state: PipelineState) -> PipelineState:
        ...
        if not active_config.mask_image or not current_lines:
            logger.warning(
                "No mask image provided or no incoming lines to process. Skipping modification."
            )
            return state
        ...
        sampler = ImageSampler(active_config.mask_image,
                               current_boundary.bounds)
