...
class TurbulenceConfig(BaseModel):
    amplitude: float = Field(default=3.0,
                             description="How strong the warping effect is")
    frequency: float = Field(default=0.1,
                             description="How tightly the ripples bunch up")
    asymmetry: float = Field(
        default=1.37,
        description=
        "Multiplier to separate X/Y phase so the distortion isn't perfectly diagonal"
    )
    phase: float = Field(default=0.0,
                         description="Offset for animation or variation")
...
class TurbulenceMod(PipelineOperation):
    ...
    def process(self,
        ...
        logger.info(
            f"Applying asymmetric turbulence (amp: {amp}, freq: {freq})...")
        ...
        for line in current_lines:
            ...
            for x, y in line.coords:
                ...
                dx = math.sin((x * freq) + phase) * math.cos(
                    y * freq * asym) * amp
                dy = math.sin(
                    x * freq * asym) * math.cos((y * freq) + phase) * amp
