...
class RadialRippleConfig(CenteredPluginConfig):
    frequency: float = Field(default=0.2, description="How many ripples per unit.")
...
class RadialRippleMod(PipelineOperation):
    ...
    def process(self, state: PipelineState, context=None) -> PipelineState:
        ...
        cx = ctx.local_center_x if ctx.local_center_x is not None else (
            cfg.center_x if cfg.center_x is not None else state.boundary.centroid.x)
        cy = ctx.local_center_y if ctx.local_center_y is not None else (
            cfg.center_y if cfg.center_y is not None else state.boundary.centroid.y)

        logger.info(f"Applying radial ripples (amp={cfg.amplitude}) from ({cx}, {cy}).")
