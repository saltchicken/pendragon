...
from pendragon.core import BasePluginConfig, PipelineOperation, PipelineState, PipelineContext
from pendragon.core.registry import OPERATION_REGISTRY
from pendragon.core import register_operation

class DistributeConfig(BasePluginConfig):
    generator: str = Field(default="grid_lines", description="Generator to stamp along the line.")
    ...
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        for line in state.lines:
            ...
            for i in range(num_stamps):
                ...
                cell_ctx = PipelineContext(local_center_x=point.x, local_center_y=point.y)
                
                res = sub_gen.process(PipelineState(boundary=local_boundary, lines=[]), context=cell_ctx)
