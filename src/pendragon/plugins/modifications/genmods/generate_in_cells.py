...
class ImageModulatorConfig(BaseModel):
    ...
    target_setting: str = Field(
        ..., 
        description="The parameter name in the sub-generator to modulate (e.g., 'revolutions', 'spacing')."
    )
    min_val: float = Field(
        ..., 
        description="The setting value when the image is pure white (0% darkness)."
    )
    max_val: float = Field(
        ..., 
        description="The setting value when the image is pure black (100% darkness)."
    )
    ...
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        if cfg.image_modulator and cfg.image_modulator.image_path:
            if not state.boundary or state.boundary.is_empty:
                logger.warning("No active boundary to map the image to. Skipping image modulation.")
            else:
                logger.info(f"Modulating '{cfg.image_modulator.target_setting}' based on {cfg.image_modulator.image_path}")
                sampler = ImageSampler(cfg.image_modulator.image_path, state.boundary.bounds)
