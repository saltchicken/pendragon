...
class PolygonizeMod(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        if not polygons:
            logger.error("Could not form any closed polygons from the current lines!")
            return state
        ...
        return PipelineState(boundary=state.boundary, lines=new_boundary_lines, operation_name="polygonize")
