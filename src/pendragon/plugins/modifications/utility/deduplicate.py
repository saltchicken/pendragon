...
class DeduplicateConfig(BaseModel):
    tolerance: float = Field(default=1e-5, description="Tolerance for exactly equal geometries.")
...
class DeduplicateMod(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        for i, line in enumerate(current_lines):
            if i in duplicates_to_skip: continue
            ...
            for j in query_indices:
                ...
                if (line.equals_exact(other_line, tolerance) or line.equals_exact(other_line.reverse(), tolerance)):
                    duplicates_to_skip.add(j)

        logger.success(f"Deduplication complete. Removed {len(duplicates_to_skip)} duplicate lines.")
        return PipelineState(boundary=state.boundary, lines=unique_lines, operation_name="deduplicate")
