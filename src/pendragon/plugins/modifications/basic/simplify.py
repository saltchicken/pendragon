...
class SimplifyConfig(BaseModel):
    tolerance: float = Field(default=0.1, ge=0.0, description="Max allowed distance between original and simplified line.")
...
class SimplifyMod(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        for line in current_lines:
            ...
            elif isinstance(simplified, LineString): simplified_lines.append(simplified)
            elif isinstance(simplified, MultiLineString): simplified_lines.extend(list(simplified.geoms))

        logger.success(f"Simplification complete. Yielded {len(simplified_lines)} lines.")
        return PipelineState(boundary=state.boundary, lines=simplified_lines, operation_name="simplify")
