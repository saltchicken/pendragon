...
def optimize_paths_nearest_neighbor(paths: List[List[Tuple[float, float]]], start_pt: Tuple[float, float] = (0.0, 0.0)) -> List[List[Tuple[float, float]]]:
    if not paths: return []
    ...
    for _ in range(n_paths):
        ...
        while best_idx == -1:
            ...
            if query_k == 1: distances, indices = [distances], [indices]
            for dist, idx in zip(distances, indices):
                ...
                if not visited[path_idx]:
                    ...
                    break
            if best_idx == -1:
                if query_k == n_paths * 2: break
                k *= 4

        if best_idx == -1: break
        path_idx, is_end = best_idx // 2, (best_idx % 2 != 0)
        chosen_path = valid_paths[path_idx]
        if is_end: chosen_path = list(reversed(chosen_path))
...
class OptimizeMod(PipelineOperation):
    def process(self, state: PipelineState, context: Optional[PipelineContext] = None) -> PipelineState:
        ...
        start_x = ctx.variables.get("start_x", ctx.local_center_x if ctx.local_center_x is not None else cfg.start_x)
        start_y = ctx.variables.get("start_y", ctx.local_center_y if ctx.local_center_y is not None else cfg.start_y)
        ...
        return PipelineState(boundary=state.boundary, lines=optimized_lines, operation_name="optimize")
