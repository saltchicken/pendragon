...
@register_operation("duplicate_translate",
                    config_class=DuplicateTranslateConfig)
class DuplicateTranslateMod(PipelineOperation):
    ...
    def process(self,
        ...
        logger.info(
            f"Duplicating lines {copies} times with offset ({tx}, {ty}) per copy..."
        )
        ...
        for i in range(1, copies + 1):
            ...
            for line in current_lines:
                ...
                duplicated_line = translate(line,
                                            xoff=current_tx,
                                            yoff=current_ty)
                all_lines.append(duplicated_line)

        logger.success(
            f"Duplication complete. Total lines went from {len(current_lines)} to {len(all_lines)}."
        )
